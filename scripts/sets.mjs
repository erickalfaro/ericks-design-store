// Single source of truth for the 12 curated icon sets.
// Used by fetch-icons.mjs, icon.mjs and build-gallery.mjs.
//
// `prefix` is the Iconify collection prefix (also the @iconify-json/<prefix> package
// and the `<prefix>:<name>` reference used by Iconify components).
// Lucide Animated is intentionally NOT here — it ships as React/Motion components,
// not static SVGs, so it lives in vendor/lucide-animated and docs/lucide-animated.md.
...
export const SETS = [
  {
    ...
    defaults: { size: 24, stroke: 2 },
    style:
      'Clean, consistent open-stroke line icons (a Feather fork). The shadcn/ui default. Best all-round pick for modern web apps.',
    whenToUse:
      'Default choice for clean line UIs, dashboards, SaaS, shadcn-based projects. Adjustable stroke width.',
  },
  {
    prefix: 'ph',
    name: 'Phosphor',
    ...
    defaults: { size: 256, stroke: null },
    style:
      'Flexible family with six weights: thin, light, regular, bold, fill, duotone. Friendly, rounded geometry.',
    whenToUse:
      'When you want one icon vocabulary but need to vary visual weight/tone (e.g. thin for elegant, bold for emphasis, duotone for accents).',
  },
  {
    prefix: 'tabler',
    name: 'Tabler Icons',
    ...
    whenToUse:
      'When you need breadth (5,900+ icons) in a clean line style and Lucide is missing a glyph. Matches Lucide visually.',
  },
  {
    prefix: 'heroicons',
    name: 'Heroicons',
    ...
    defaults: { size: 24, stroke: null },
    style:
      'Small, polished, curated set by the Tailwind team. Outline (24px), Solid (24px), Mini (20px), Micro (16px).',
    whenToUse:
      'Tailwind / Tailwind UI projects, or when you want a tight, opinionated, beautifully balanced set over sheer count.',
  },
  {
    prefix: 'radix-icons',
    name: 'Radix Icons',
    ...
    docs: 'https://www.radix-ui.com/icons',
    npm: '@radix-ui/react-icons',
    defaults: { size: 15, stroke: null },
    style:
      'Crisp 15×15 UI glyphs designed for dense interface chrome (menus, toolbars, controls).',
    whenToUse:
      'Radix UI / Radix Themes projects and compact application chrome where icons sit at small fixed sizes.',
  },
  {
    prefix: 'feather',
    name: 'Feather',
    ...
    whenToUse:
      'Legacy or minimalist projects already on Feather. For new work prefer Lucide (actively maintained superset).',
  },
  {
    ...
    whenToUse:
      'Android, Google-ecosystem, or Material Design 3 products, or when you need an enormous, official, system-grade catalog.',
  },
  {
    prefix: 'ri',
    name: 'Remix Icon',
    ...
    whenToUse:
      'When you want paired line+fill variants (e.g. active/inactive tab states) and good logo/brand glyph coverage.',
  },
  {
    prefix: 'bi',
    ...
    npm: 'bootstrap-icons',
    defaults: { size: 16, stroke: null },
    style:
      'Official Bootstrap set. Mostly solid/filled forms on a 16px grid; many have outline+fill pairs.',
    whenToUse:
      'Bootstrap-based projects, or when you want filled glyphs that read well at small sizes.',
  },
  {
    prefix: 'iconoir',
    name: 'Iconoir',
    ...
    docs: 'https://iconoir.com',
    npm: 'iconoir / iconoir-react / @iconoir/vue',
    defaults: { size: 24, stroke: 1.5 },
    style:
      'Large hand-crafted 1.5px-stroke line set with a slightly softer, editorial feel than Lucide/Tabler.',
    whenToUse:
      'Marketing sites and products wanting a refined, lighter-stroke line aesthetic distinct from the 2px crowd.',
  },
  {
    prefix: 'bx',
    name: 'Boxicons',
    ...
    whenToUse:
      'When you need both line and solid versions plus a big library of brand/social logos in one set.',
  },
];

export const SET_BY_PREFIX = Object.fromEntries(SETS.map((s) => [s.prefix, s]));
